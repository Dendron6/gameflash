...
describe('Game Flash For You Test Suite', function () {
    ...
    it(`Linkage test, switch to ${action.langRu} page`, async () => {
        global.expectedUrlRu = await action.findCurrentUrl(action.flag1);
        expect(expectedUrlRu).to.be.equal('https://gf4y.com/', `This is URL of ${action.langRu} page`)
    ...
    it(`Linkage test, switch to ${action.langGer} page`, async () => {
        global.expectedUrl = await action.findCurrentUrl(action.flag3);
